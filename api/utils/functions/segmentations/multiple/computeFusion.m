...
function resSeg = computeFusion(segmentations, fusionAlgorithm, closeLineAlgorithm, varargin)
    printName = erase(fusionAlgorithm, 'algorithm_');
    ...
    if length(segmentations) < 2
        throw(MException('TDSFT:algorithms', 'The number of segmentations must be at least 2'));
    end
